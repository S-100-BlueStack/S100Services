...
const PRODUCT_MANAGER_ROOT = fileURLToPath(new URL("../../../../", import.meta.url));
...
test("user-facing Product Manager sources use Product terminology", async () => {
  const violations = [];

  for (const relativePath of USER_FACING_SOURCE_FILES) {
    const absolutePath = path.join(PRODUCT_MANAGER_ROOT, relativePath);
