...
Product Manager runtime baseline: `785567b4440129ea192798f0199d44a5cee94289`.
...
Product Manager frontend
        ...
        v
ProductManagerAPI
  - keeps public Product Manager endpoints
  - validates and enqueues Product Manager jobs
...
Shared Hangfire storage
        ...
        |
JobPlatform.Worker
  - later executes Product Manager jobs
  - may later host selected scheduled tasks
```

The Product Manager API remains in the current application. Only Hangfire Server/worker hosting is a candidate for later extraction to the separate JobPlatform solution.

## 2. Review boundary

### Included

- Current Product Manager Hangfire client, server, status and job execution dependencies.
- Portability requirements for executing Product Manager jobs in `JobPlatform.Worker`.
...
- Adding Product Manager references to JobPlatform.
...
## 3. Current Product Manager job architecture
...
- Product Manager job-status reader;
- recurring-job registration host;
- Product Manager HTTP API.
...
This keeps the frontend isolated from JobPlatform deployment details and prevents the shared job platform from becoming a Product Manager domain API.
...
ProductManagerAPI reads Hangfire state and Product Manager job parameters directly from shared Hangfire storage. This can continue after worker extraction provided ProductManagerAPI retains compatible read access to the same storage and serializer contract.
...
## 4. Product Manager worker dependency inventory
...
- Product Manager/S-128 and system database connections;
- Hangfire SQL storage connection;
- ArcGIS geodatabase and service configuration;
- connection files;
- Product Manager output folder;
...
No implementation should assume that copying `appsettings.json` alone recreates the Product Manager execution environment.
...
- worker extraction must not enable Product Manager execution on both the old API host and the new worker host at the same time;
- a multi-worker rollout must not rely on machine-local lock files;
- a shared operation-ownership design must be approved before distributed Product Manager execution;
...
That structure is suitable as the future host, but Product Manager integration should be modular rather than copied directly into `JobPlatform.Worker/Program.cs`.
...
- client filters and atomic Product Manager metadata creation;
- queue name and queue ordering;
- terminal-state and retention behavior.

Existing queued Product Manager jobs serialize the current type and method identity. A deployment that removes or relocates those types without a compatibility bridge can strand queued jobs.

## 8. Queue recommendation for later implementation

Product Manager jobs should later use a dedicated queue, provisionally:
...
- Product Manager jobs require Windows/x64/ArcGIS dependencies not needed by every JobPlatform job;
...
- Product Manager jobs should not be consumed by generic workers that lack required runtime assets.
...
- it does not currently use the Product Manager dataset-lock/job-execution framework consistently;
...
- mutation scope;
- concurrency interaction with Product Manager operations;
...
- Product Manager HTTP routes and response models;
...
- atomic Product Manager creation metadata;
- frontend-compatible job status and active-operation endpoints;
- Product Manager authorization when production security is enabled.

### JobPlatform.Worker later becomes responsible for

- consuming the Product Manager queue;
- resolving Product Manager job implementation dependencies;
...
1. Record exact Product Manager and JobPlatform commits.
2. Freeze runtime contract changes during cutover preparation.
3. Add compatible Product Manager job assemblies/module to JobPlatform.
4. Configure a dedicated Product Manager queue without consuming it yet.
5. Verify worker startup, DI, ArcGIS runtime, database and filesystem access.
6. Pause new Product Manager job creation or enter a controlled maintenance window.
7. Drain or explicitly resolve existing Product Manager jobs in the old storage/queue.
8. Deploy ProductManagerAPI as client/status reader without Hangfire Server.
9. Enable the Product Manager queue on JobPlatform.Worker.
...
- stop Product Manager queue consumption in JobPlatform before restarting the old worker;
- verify no Product Manager job is Processing;
...
1. Whether Product Manager keeps its current Hangfire database or moves to JobPlatform storage.
2. Which application owns Hangfire schema migrations and retention.
3. Exact shared assembly/project structure.
4. Final Product Manager queue name and worker concurrency.
...
12. Production authentication/authorization for Product Manager job endpoints.

## 15. Review conclusion

The current async Product Manager design is portable to a separate JobPlatform worker without changing the frontend or public ProductManagerAPI routes.

The move is **not** a connection-string-only change. The future worker must reproduce the Product Manager execution environment, and distributed concurrency must be resolved before more than one host can execute Product Manager mutations.
