...
Baseline: `345b79eef2a9225473d57db80243e731739cbc3a`.
Status: Implemented, uncommitted; Windows build/browser acceptance pending.
...
| FI-009/010/012–015/017–019  | Existing accepted behavior preserved; focused Windows regression pending.                                                                                        |
| Normalized smoke hardening  | Source-local loading feedback, early persisted theme application and selected-source Cancel Export gating are implemented in the current uncommitted correction. |
...
| BE-007 | Jobs     | External shared Hangfire worker migration              | Implemented; dev-server validation pending  | API and worker now run as mutually exclusive process roles with separate ArcGIS/ProductManager lanes and one background Hangfire worker. Windows Service hosting is provided for the Worker role; LocalSystem/CoreHost/service-restart validation remains a deployment acceptance gate. |
...
| FI-022 | Analyze / Review                 | Refresh open workspace Product content after backend changes             | Todo                                     | Analyze and Review currently refresh when route/Product composition changes, but an already-open workspace does not discover later validation artifacts, Product metadata or History changes. Add a compact manual Refresh and/or targeted refresh after known Product-job completion. Preserve source-aware resolution, generation guards, independent content failures, enabled Product composition and user-selected Review content. Avoid blind polling unless a cheap backend freshness contract is available. |
...
| FI-024 | Analyze / Review performance     | Resolve workspace Product source without loading complete AOI catalogs   | Implemented; local verification pending  | Analyze/Review now use `GET /electronicproducts` only for lightweight dataset-name picker choices, then resolve each selected Product through `GET /electronicproducts/{datasetName}/aoi`. The targeted backend response carries authoritative `ProductSpecification`; identity conflicts fail closed. No S-57/S-101 dataset-name heuristic or bulk-AOI fallback is used.                                                                                                                                           |
| FI-025 | Backend integration              | Isolate long-running export ArcGIS work from interactive API reads       | Implemented; local verification pending  | Backend process-isolation candidate separates API `Interactive` ArcGIS ownership from one serialized Hangfire `Background` worker process. Frontend retries/timeouts are not used to mask starvation. Production-equivalent two-process smoke verification remains required before acceptance.                                                                                                                                                                                                                      |
...
17. Add FI-022 workspace refresh without introducing blind polling; prefer explicit user refresh and known job-completion events unless the backend exposes a cheap freshness signal.
...
19. Verify FI-024 locally: Analyze/Review must make no S57/S101 bulk AOI calls and targeted resolution must remain fail closed without dataset-name heuristics.
20. Verify FI-025 with the production-equivalent API/worker split before acceptance; do not mask ArcGIS execution starvation with frontend polling, retry or timeout changes.
