...
| FI-022 | Analyze / Review                 | Keep open workspace Product content automatically fresh after backend changes | Todo                                     | Analyze and Review currently refresh when route/Product composition changes, but an already-open workspace does not discover later validation artifacts, Product metadata or History changes. Automatic freshness is the target: refresh affected Product content immediately after known Product-job completion and use a lightweight backend freshness signal to discover changes from other users/processes when such a signal can be implemented without ArcGIS or expensive content reads. Poll only the cheap signal while the workspace is active, recheck on foreground/focus, and refresh only changed Products. Keep a compact manual Refresh as fallback/recovery. Preserve source-aware resolution, generation guards, independent content failures, enabled Product composition and user-selected Review content. Do not poll full AOI/Analyze/History/artifact payloads or introduce push infrastructure solely for FI-022. |
...
17. Implement FI-022 as automatic-first workspace freshness: use known Product-job completion for immediate targeted refresh and, if backend discovery confirms a cheap persisted freshness signal, poll only that signal while Analyze/Review is active and recheck on foreground/focus; keep manual Refresh as fallback. Do not poll full workspace payloads or introduce push infrastructure solely for this feature.
...
## FI-022 automatic Analyze/Review workspace freshness

Status: Todo

### Goal

Keep already-open Analyze and Review workspaces current when relevant Product data changes after the initial load. Normal operation should not require the user to press Refresh. A manual Refresh remains available as an explicit fallback and recovery action.

Automatic freshness must preserve the existing source-aware workspace architecture, FI-024 targeted Product resolution, generation/stale-response guards, independent per-content failure handling, enabled Product composition, and user-selected Review content.

### Preferred refresh model

Use the smallest available signal for each change source:

1. **Known Product-job completion:** when existing frontend job tracking observes a relevant Product job reach a terminal state, immediately refresh only that Product in any open Analyze/Review workspace. Job status is a refresh trigger, not the authoritative freshness source.
2. **Changes from other users or processes:** if backend discovery confirms a cheap persisted freshness contract, check that contract at a bounded interval while the workspace is active and refresh only Products whose freshness token changed.
3. **Foreground/focus:** when a hidden or backgrounded Analyze/Review workspace becomes active again, perform an immediate freshness check before resuming the normal bounded cadence.
4. **Manual recovery:** expose a compact manual Refresh that can re-request the current workspace content when automatic detection is unavailable, delayed, or the user explicitly wants to retry failed content.

Do not continuously poll full AOI, Analyze, Product History, validation-artifact, or report payloads merely to detect change. Do not introduce SignalR, WebSockets, SSE, a message bus, or another push architecture solely for FI-022 unless a later cross-application requirement independently justifies it.

### Backend freshness discovery

Before fixing a public freshness endpoint contract, inspect the normalized Product workflow persistence and the current workspace content sources. Prefer a small read-only repository/database contract that:

- accepts one or more globally unique `datasetName` values so one Review workspace does not need one freshness request per Product;
- returns a stable Product-level token or a small set of component tokens only when that distinction is already cheap and authoritative;
- changes whenever persisted Product metadata/state, Product History, or validation-artifact availability relevant to Analyze/Review changes;
- performs no ArcGIS/ProductManager geometry work and does not load AOI, Analyze results, report content, or other large payloads;
- does not use Hangfire job state as the source of truth for whether Product content is current;
- requires no new distributed ownership, event-stream, or broad persistence architecture.

If the current backend cannot provide such a signal cheaply from authoritative persisted state, document that result before selecting a reduced fallback. Do not silently replace freshness checks with periodic reloads of the full workspace content.

### Frontend lifecycle and acceptance direction

The eventual implementation should:

- stop periodic freshness checks when the workspace is hidden, torn down, or superseded;
- abort or ignore stale checks and stale Product reloads through the existing generation/session model;
- coalesce overlapping freshness checks and targeted reloads for the same Product;
- retain last successful content when one refreshed content area fails, consistent with the existing independent-content failure boundary;
- preserve Product ordering, enabled Product composition, route state, expanded/collapsed state where currently preserved, and user-selected Review content;
- keep unsupported source workflows fail closed without compatibility or cross-source fallback;
- preserve FI-024 targeted Product AOI resolution and FI-025 API/worker ArcGIS process isolation;
- provide a compact accessible manual Refresh fallback without making it the normal freshness mechanism.
