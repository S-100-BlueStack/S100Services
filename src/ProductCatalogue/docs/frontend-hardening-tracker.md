...
| FI-021 | Main map / Overlap selection   | Add modifier-click direct selection for the highlighted Product candidate | Ready for discovery and implementation   | Current design-review scope replaces the older Product Collection toggle experiment: normal click keeps the overlap picker, while Ctrl/Cmd-click should directly select the currently highlighted Product through the existing Product-context/popup path. Do not hard-code source preference; Shift-click remains unassigned. See `ui-ux-design-backlog.md`. |
...
| FI-026 | Shared navigation / Preferences    | Consolidate active-route state and shared navbar controls                 | Ready                                    | Persist the active navigation underline, reorder Main-map controls, show compact last-updated time, hide Help, and move Light/Dark selection into Preferences. Implement shared behavior once across routes. See `ui-ux-design-backlog.md`. |
...
21. Use `ui-ux-design-backlog.md` as the detailed design source for FI-021 and FI-026 through FI-039.
22. Take FI-021 next, using modifier-click for direct highlighted Product selection while preserving the normal overlap picker and source-aware Product context.
...
Status: Ready for discovery and implementation  
...
- Ctrl-click on Windows/Linux and Cmd-click on macOS directly selects the Product candidate currently highlighted by the existing candidate/hover ordering;
...
- Shift-click remains unassigned because the map has no stable range-selection model;
- keyboard users require an equivalent activation path;
- Product Collection behavior remains unchanged.

Before implementation, identify the owner of current candidate highlight/order and verify that the modifier can be consumed without conflicting with ArcGIS map gestures. Define the visible discoverability cue and behavior when no candidate is highlighted.
