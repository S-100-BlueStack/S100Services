...
Current reviewed repository baseline: `fce91ee3ee54361f3a0fe79e3dd95c8c1905a718`.
...
| FI-031 | Main map / Data sources          | Prevent startup/reload with zero enabled Product sources                  | Implemented; manual verification pending | Controller initialization now distinguishes first visits from valid restored selections. A restored selection resolving to zero enables exactly one currently eligible selection-persistable source through registry default/definition ordering and the established activation/persistence lifecycle; session-only synthetic fixtures remain valid first-visit defaults but are not restored-state fallbacks. Stale or unavailable saved IDs cannot block eligible recovery, while startup truthfully remains at zero when no eligible source exists. In-session all-off remains valid, and generation/revision guards prevent an older startup fallback from overwriting newer user state. No source IDs or names are hard-coded. FI-026/FI-028 behavior is unchanged. See `ui-ux-design-backlog.md`. |
| FI-032 | Main map navigation              | Open workspaces in new tabs and correct false popup-blocked notices       | Ready                                    | Main-map launches of Dashboard/Analyze/Review open a new tab; navigation from those routes remains same-tab. Product Collection must not show a blocked-popup notice after a successful open. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| FI-033 | Dashboard                        | Compact page chrome and the range/refresh toolbar                         | Ready                                    | Remove redundant headings/range copy and old presets, replace text Refresh with icon + last-load time, remove the separate To Clear action, and improve From/To label contrast while preserving paging/sort/history contracts. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| FI-034 | Dashboard                        | Auto-apply valid filter changes                                           | Ready after FI-033                       | Remove Apply; refresh on committed valid changes, keep text debounce, last-successful results, stale-request guards, page reset, and cursor invalidation. Saved filter presets remain deferred. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| FI-035 | Analyze                          | Compact the sidebar and align scrollbar/list presentation                 | Ready                                    | Remove redundant visible copy, reduce spacing/list height/sidebar width, rename Product list to Products, and reuse shared scrollbar styling while preserving FI-022/FI-024 behavior. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| FI-036 | Review                           | Compact sidebar controls and move Refresh above Products                  | Ready                                    | Remove redundant workspace/add-product copy while retaining accessible/contextual messaging, and move manual Refresh near the Product composition controls. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| FI-037 | Review                           | Reduce Product-content density and nested report scrolling                | Ready after FI-036                       | Make History sizing more content-sensitive, let Product/content own scrolling, and simplify duplicated IC-ENC/validation empty-state copy without collapsing materially different backend states. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| FI-044 | Shared Preferences / UX          | Clarify Preferences information architecture and saved-state controls     | Ready                                    | Separate Introduction, actual Settings, and Saved preferences so runtime values are not confused with browser persistence. Align Theme and Scale hiding presentation, give saved Scale hiding a consistent clear/reset affordance, define unambiguous reset semantics, and preserve FI-026/FI-028 route/state contracts. FI-043 must audit onboarding against the final Preferences structure. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                           |
...
14. FI-012, FI-009, FI-013, FI-014, FI-015, FI-017, FI-021, FI-022, FI-026 and FI-028 are complete; the latest accepted frontend baseline is `fce91ee3ee54361f3a0fe79e3dd95c8c1905a718`.
...
25. FI-031 non-empty source startup is implemented with manual verification pending. Next take FI-032 Main-map workspace navigation; FI-027 navbar search still requires its short design pass after the shared navbar structure is stable.
26. Complete Dashboard work as FI-033 then FI-034 so visual compaction is stable before Apply is removed.
27. Complete Analyze compaction as FI-035 without changing FI-022 freshness or FI-024 targeted resolution; use or establish a shared/public scrollbar styling contract that FI-042 can reuse.
28. Apply FI-042 Filter-panel scrollbar consistency after the shared scrollbar treatment is established.
29. Complete Review work in controlled order: FI-036 sidebar compaction, FI-041 Product-list scroll preservation, FI-037 content layout/empty states, FI-038 defaults/bulk toggles, then FI-039 incremental composition loading.
...
32. FI-044 Preferences information-architecture/usability work may proceed as a bounded shared-UI task after the immediate FI-031/FI-032 behavior items; it is not a blocker for them, but it should settle before FI-043 audits final onboarding.
...
The bounded FI-021 pointer implementation does not satisfy the original keyboard-equivalent activation or visible discoverability-cue requirements. Those requirements remain active and are deferred explicitly to FI-040 rather than treated as implemented. FI-040 must preserve normal overlap-picker access and cannot make modifier selection a prerequisite for using the application.
