...
Current reviewed repository baseline: `e235a15b6d86dc3c5313a4a151fa8c5146612a3a`.
...
| FI-036 | Review                           | Compact sidebar controls and move Refresh above Products                  | Implemented; manual verification pending | Removed redundant visible Review/sidebar Product-picker copy while retaining accessible and contextual messaging. Review explicitly opts into an anchored Product-picker results overlay, and the existing single manual Refresh action is icon-only in the `Products` header immediately before the counter. Its event, accessibility, loading/disabled rules and FI-022 lifecycle are unchanged. Product composition, content toggles, FI-024 targeted resolution, source boundaries, routing, independent failures and the accepted shared `pc-scrollbar` contract remain unchanged. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| FI-041 | Review / Product list            | Preserve Product-list scroll position when content toggles change         | Implemented; manual verification pending | History, IC-ENC and Validation toggles capture the current Product-list `scrollTop` plus stable Product/content-type focus identity before their synchronous authoritative rerender, then restore the replacement list and toggle with `preventScroll`. The snapshot is call-local and never reused by add/remove, Product enable/disable, route replacement, Refresh, FI-022 freshness or teardown, so repeated toggles cannot accumulate stale restoration work. Product ordering, selection, source identity, independent failures and keyboard behavior remain unchanged. See `ui-ux-design-backlog.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
29. Manually verify the implemented FI-036 sidebar compaction and FI-041 Product-list interaction preservation before continuing with FI-037 content layout/empty states, FI-038 defaults/bulk toggles, and FI-039 incremental composition loading.
