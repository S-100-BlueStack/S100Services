...
Documentation baseline: `01f22a605e8e4d29a8da187306af1dc04b8540ef`  
...
| FI-034 | Dashboard                       | Auto-apply valid filter changes and remove the Apply action                                           | Implemented; manual verification pending  | Apply is removed; committed valid date/time changes auto-apply through the existing request lifecycle. Invalid drafts do not request or update the route. Discrete filters remain immediate, search remains debounced, and paging/cursor reset, sort/page size, last-successful results, FI-033 `HH:MM`, manual Refresh, History and FI-032 navigation are preserved.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| FI-035 | Analyze                         | Compact the Analyze sidebar and align scrollbar/list presentation                                     | Implemented; manual verification pending  | Preserves Product picker behavior, FI-022 freshness, source-aware resolution, manual Refresh, independent content failures, and keyboard behavior while establishing the neutral shared `pc-scrollbar` contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
The implementation removes the duplicate visible panel heading and normal Product-picker label/help
while keeping public accessible names and contextual loading, failure, and validation messages. The
sidebar is narrower, the dataset-manager spacing is reduced, and the `Products` list has a responsive
viewport for approximately three normal rows without fixing row height. The shared `pc-scrollbar`
class now owns the previously Review-specific scrollbar appearance and is applied to the existing
Review scroll surfaces plus the Analyze outer scroller and Product list. No Analyze routing, loading,
source resolution, freshness, Refresh, or failure lifecycle changed.
...
5. FI-033 Dashboard visual compaction is complete and manually accepted at `01f22a605e8e4d29a8da187306af1dc04b8540ef`; FI-034 automatic filtering is the next bounded Dashboard implementation.
