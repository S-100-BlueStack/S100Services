...
| FI-039 | Review performance              | Load only changed/new Products when Review composition changes                                        | Done                                      | Manually accepted at `1a77af904ec1a41726bcec357b72610c74e793ca`. One Review Product session owns payload publication; additions load only new Products, removals and ordinary visibility changes retain survivors, manual Refresh remains full, and FI-022 reloads changed eligible Products only. Targeted freshness acknowledgement fails closed for ineligible changed sets, while a composition-observation epoch rejects stale FI-022 results across remove/re-add membership boundaries. FI-038 intent/selections, FI-041 call-local snapshots and independent failures remain preserved. See `fi-039-incremental-review.md`.                                                                                                                                                        |
| FI-040 | Main map / accessibility        | Add modifier-selection keyboard equivalent and discoverability                                        | Done                                      | Manually accepted at `33f0089810b1ba8c8a053727e8828f5b519e66c9`. Map-focused Ctrl/Cmd+Enter revalidates the transient highlighted Product against current interactive layers/Graphics and opens it through the same popup path as FI-021. A compact contextual hint exposes pointer and keyboard shortcuts; normal overlap selection remains complete.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
The bounded FI-021 pointer implementation remains unchanged. FI-040 separately implements and has manually accepted its keyboard equivalent and visible discoverability cue while retaining the same transient-highlight ownership and normal overlap-picker workflow at `33f0089810b1ba8c8a053727e8828f5b519e66c9`.
...
At FI-038 acceptance, FI-039 remained unimplemented. See the FI-039 implementation update below.
...
Status: **Done**.  
Implementation commit: `1a77af904ec1a41726bcec357b72610c74e793ca`.

Implementation baseline: `1033721e4c6300bb92dd66a96dc072fda04fb152`.
The implementation and verification record is [FI-039 incremental Review](fi-039-incremental-review.md).
The observations and requirements below describe the baseline problem and retained acceptance criteria.
...
FI-039 was manually accepted on 2026-09-18 and committed at `1a77af904ec1a41726bcec357b72610c74e793ca`. The accepted implementation keeps `createReviewProductSession` as the sole Review Product payload-generation owner; sequential additions load only newly added Products, removals and ordinary Product visibility changes do not refetch survivors, manual Refresh remains a full enabled-composition reload, and FI-022 continues changed-only refresh through the same session. Acceptance also includes the final freshness race safeguards: targeted acknowledgement is rejected when the requested changed set is no longer fully eligible, and composition-observation epochs prevent pre-mutation FI-022 observations from crossing remove/re-add membership boundaries. FI-038 content intent/selections, FI-041 scroll/focus, FI-036/FI-037 presentation and independent content failures remain preserved. The accepted repository commit also contains colleague merge changes and is therefore the authoritative merged post-FI-039 baseline. The local frontend check passed and formatting was run before commit.
...
- the public MapView `key-down` surface owns Ctrl+Enter on Windows/Linux and Cmd+Enter on macOS;
- the shortcut reads Ctrl/Cmd/Shift from that triggering event's native KeyboardEvent and ignores
  repeated keydown events, with no document listener or sticky modifier state;
- activation snapshots the same transient highlighted Product identity exposed to FI-021 and never
  promotes popup-locked highlight state into a keyboard candidate;
- current overlap-enabled layers and their current public Graphic collections are re-read at
  activation time, then validated through the existing current-layer/Graphic-membership rules;
- pointer and keyboard direct activation share the stable highlighted-identity resolver and the same
  normal Product popup publication callback; no synthetic click or second popup-selection path is
  introduced;
- the pointer click and keyboard accelerator share one interaction generation, so a keyboard action
  supersedes an older in-flight click and teardown retires both paths;
- no highlighted identity, hidden/removed Graphic, source/layer deactivation, stale membership,
  superseded generation, or missing popup location results in Product activation. A current
  replacement Graphic with the same stable identity can remain eligible, matching FI-021's accepted
  replacement semantics;
- Shift-click, Product Collection, Product search, Locator, overlap ordering/navigation, Escape and
  popup focus behavior are unchanged.

### Discoverability and accessibility

A compact application-owned note appears in the public MapView bottom-left UI only while a transient
highlighted candidate exists:

```text
Ctrl/Cmd-click or Ctrl/Cmd+Enter to open highlighted Product
```

The cue uses the existing theme tokens, has no animation or floating-action treatment, exposes the
same text through native title/help and accessible note naming, and is hidden/removed with the
transient-highlight/interaction lifecycle. It deliberately does not use `aria-keyshortcuts`, because
the cue is not itself focusable or the owner of the keyboard event. The shortcut acts only through
the map's public keyboard event surface.

### Preservation boundary

Normal click remains the complete zero/one/multiple-candidate workflow and the overlap picker remains
fully usable without any modifier. FI-021 Ctrl/Cmd-click still snapshots before asynchronous hit
testing and retains its post-hitTest revalidation/fallback behavior. Source identity remains derived
from stable Product/Graphic metadata, never source names or DatasetName heuristics.

### Manual acceptance

FI-040 was manually accepted on 2026-09-18 and implemented at `33f0089810b1ba8c8a053727e8828f5b519e66c9`. Acceptance confirmed Ctrl/Cmd+Enter only on the map interaction surface, transient-highlight current-state revalidation, shared pointer/keyboard direct-selection publication, repeat/Shift fail-closed behavior, contextual shortcut-hint lifecycle, unchanged overlap picker/Product Collection, and acceptable light/dark/responsive presentation. The current documentation source is `8acfec141ff508fde5484119af7a3f01ab084bd7`, preserving the subsequent merge and API documentation changes.
...
Status: **Implemented; manual verification pending**. The requirements below are retained as the design record.
See [the completed baseline audit, route coverage and verification](fi-043-onboarding-refresh.md).
...
This makes repeated names such as `Theme` and `Scale hiding` appear to mean the same thing even though one control changes runtime behavior and the other controls persistence.

### Information architecture

Organize the panel into three compact semantic concepts without redundant visible headings:

```text
Start introduction

Theme
Scale hiding (Main map)

Saved preferences
  Auto-save / Reset by supported preference
...
- Auto-save is the only saved-state control: OFF removes that preference's stored value without changing runtime; ON retains the existing owner-specific persistence behavior.
- Do not expose a separate saved-value status or per-item clear action.
- Reset restores the owner's accepted default/runtime state and never changes Auto-save.
...
- compact Introduction action, runtime settings and Saved preferences ordering without redundant visible Introduction/Settings headings;
...
- Auto-save removal and Reset remain independent for Theme and Scale hiding;
...
9. FI-038 remains accepted at `54a3f89e83ad18c18875921f562f81c58b9d3d8d`, and FI-039 incremental Review reconciliation is complete and manually accepted at `1a77af904ec1a41726bcec357b72610c74e793ca`. Preserve its loading/reconciliation, freshness ownership, Review content intent, presentation and scroll/focus contracts.
...
3. **Introduction utility**: the existing callback replays the current route from a real secondary button at the bottom; its explanation remains hover help rather than permanent copy.
...
Reset is independent and never toggles Auto-save: Theme resets to Light and saves Light when enabled; Scale hiding resets to OFF and removes its value; Map view restores its default viewpoint. Filters Reset restores declarative first-visit defaults, including the conservative Idle exclusion, and clears stale/pending snapshot intent while remaining distinct from Filter-panel Clear all. Dashboard page-size Reset removes its record and restores 50. Reset available preferences retains the baseline scope, including Main-map source-selection defaults and Dashboard page size. No global clear was added.

Ordinary Theme, Auto-save and Reset actions update controls in place. Theme is a standard switch with visible Light/Dark icon endpoints and remains focused while the existing Theme owner applies Light/Dark. Start introduction is separated from settings as a bottom secondary button. Context-driven rendering restores application-owned focus identity. Opening, Escape, trigger close and click-away retain the v1 lifecycle. Scale hiding and Saved preferences help remain. Filters Reset now returns the filter service to its declarative first-visit defaults, including the conservative Idle exclusion, while Filter-panel Clear all retains its explicit unfiltered semantics.
