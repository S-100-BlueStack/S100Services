...
export function createProductPickerForm({
  ...
  form.className = ["pc-product-picker", className].filter(Boolean).join(" ");
  ...
  label.id = labelId;
  label.className = "pc-product-picker__label";
  label.textContent = labelText;

  const row = document.createElement("div");
  row.className = "pc-product-picker__row";

  const input = document.createElement("input");
  input.id = inputId;
  input.className = "pc-product-picker__input";
  input.type = "text";
  ...
  results.className = "pc-product-picker__results";
  results.hidden = true;
  results.setAttribute("role", "listbox");
  results.setAttribute("aria-label", "Available products");

  const help = document.createElement("p");
  help.className = "pc-product-picker__help";
  ...
  input.addEventListener("keydown", (event) => {
    ...
    if (event.key === "ArrowDown" && !results.hidden) {
      const firstOption = results.querySelector(".pc-product-picker__option");
      firstOption?.focus();
...
function createProductOption(product, onClick) {
  ...
  option.type = "button";
  option.className = "pc-product-picker__option";
...
function createStateMessage(message) {
  ...
  state.className = "pc-product-picker__state";
...
function getResultOptions(container) {
  return [...container.querySelectorAll(".pc-product-picker__option")];
}
