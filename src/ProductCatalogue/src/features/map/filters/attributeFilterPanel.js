...
function getOpenFieldKeys(panel) {
  return new Set(
    Array.from(panel.querySelectorAll(".pc-filter-field[open]")).map((details) =>
...
function updateRangePreview(panel, filterService, layerId, fieldName) {
  ...
  const summary = rangeElement.closest(".pc-filter-field")?.querySelector("[data-range-summary]");
...
function renderCheckboxField({ filterService, layerId, fieldName, values, openFieldKeys }) {
  ...
  return `
    <details class="pc-filter-field" data-layer-id="${escapeHtml(layerId)}" data-field-name="${escapeHtml(fieldName)}" ${isOpen ? "open" : ""}>
      <summary>
            ...
            return `
              <label class="pc-filter-option ${entry.count === 0 ? "pc-filter-option--empty" : ""}">
                <input
                  ...
                  ${checked ? "checked" : ""}
                />
                <span class="pc-filter-option__label">${escapeHtml(displayLabel)}</span>
...
function renderRangeField({ filterService, layerId, fieldName, values, openFieldKeys }) {
  ...
  return `
    <details class="pc-filter-field" data-layer-id="${escapeHtml(layerId)}" data-field-name="${escapeHtml(fieldName)}" ${isOpen ? "open" : ""}>
      <summary>
        <span>${escapeHtml(fieldLabel)}</span>
        <span class="pc-filter-field__summary" data-range-summary>${state.selectedValueCount}/${values.length}</span>
      </summary>
      <div class="pc-filter-field__actions">
        <button type="button" data-reset-range data-layer-id="${escapeHtml(layerId)}" data-field-name="${escapeHtml(fieldName)}">Reset</button>
      </div>
      <div class="pc-filter-range" data-range-values data-layer-id="${escapeHtml(layerId)}" data-field-name="${escapeHtml(fieldName)}">
        ...
        <calcite-slider
          ...
          ticks="0"
        ></calcite-slider>
        <div class="pc-filter-range__hint" data-range-hint>${state.featureCount} product(s) in range</div>
...
function renderLayer({ filterService, layerId, openFieldKeys }) {
  ...
  if (!fields.length) {
    return `
      <section class="pc-filter-layer">
        <h3>${escapeHtml(layerId)}</h3>
        <p class="pc-filter-empty">No filterable attributes found.</p>
      </section>
    `;
  }
        ...
        ${
          layerIds.length
            ? layerIds
                ...
                .join("")
            : '<p class="pc-filter-empty">No layers loaded.</p>'
        }
      </div>
    `;
