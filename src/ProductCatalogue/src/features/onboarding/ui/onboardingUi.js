export function createWelcomeDialog({
  ...
  overlay.className = "pc-onboarding-welcome";
  overlay.setAttribute("role", "presentation");

  const dialog = document.createElement("section");
  dialog.className = "pc-onboarding-welcome__dialog";
  dialog.setAttribute("role", "dialog");
  dialog.setAttribute("aria-modal", "true");
  dialog.setAttribute("aria-labelledby", "pc-onboarding-welcome-title");

  dialog.innerHTML = `
    <div class="pc-onboarding-welcome__eyebrow">Introduction</div>
    <h2 id="pc-onboarding-welcome-title"></h2>
    <p class="pc-onboarding-welcome__description"></p>
    <div class="pc-onboarding-welcome__actions">
      <button type="button" class="pc-onboarding-button pc-onboarding-button--secondary" data-action="dismiss">Do not show again</button>
      <button type="button" class="pc-onboarding-button pc-onboarding-button--secondary" data-action="later">Not now</button>
      <button type="button" class="pc-onboarding-button pc-onboarding-button--primary" data-action="start">Start introduction</button>
    </div>
  `;

  dialog.querySelector("#pc-onboarding-welcome-title").textContent = title;
  dialog.querySelector(".pc-onboarding-welcome__description").textContent = description;
...
export function createTourPopover({ onBack, onNext, onRequestClose }) {
  ...
  highlightLayer.className = "pc-onboarding-highlight-layer";
  highlightLayer.setAttribute("aria-hidden", "true");

  const popover = document.createElement("section");
  popover.className = "pc-onboarding-popover";
  ...
  popover.innerHTML = `
    <button
      type="button"
      class="pc-onboarding-popover__close"
      data-action="close"
    ...
    <div class="pc-onboarding-popover__actions">
      ...
      <button type="button" class="pc-onboarding-button pc-onboarding-button--primary" data-action="next">Next</button>
    </div>
  `;
  ...
  return {
    render({
      ...
      popover.querySelector(".pc-onboarding-popover__meta").textContent =
        `Step ${index + 1} of ${count}`;
      popover.querySelector(".pc-onboarding-popover__title").textContent = step.title;
      popover.querySelector(".pc-onboarding-popover__description").textContent = step.description;
...
function renderHighlights(layer, targetRects) {
  ...
  for (const targetRect of targetRects) {
    ...
    highlight.className = "pc-onboarding-highlight";
