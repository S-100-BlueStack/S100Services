...
export function createProductHistorySummary(history) {
  ...
  container.className = "pc-product-history-summary";
  ...
  container.appendChild(
    createSummaryItem({
      ...
      className: "pc-product-history-summary__item--wide",
    })
  );
...
export function createProductHistoryEventList(events) {
  ...
  list.className = "pc-product-history-list";
...
export function createProductHistoryBanner({ title, message }) {
  ...
  container.className = "pc-product-history-banner";

  const heading = document.createElement("h3");
  heading.className = "pc-product-history-banner__title";
  heading.textContent = title;

  const body = document.createElement("p");
  body.className = "pc-product-history-banner__message";
...
export function createProductHistoryStateMessage({ title, message }) {
  const container = document.createElement("div");
  container.className = "pc-product-history-state";

  const heading = document.createElement("h3");
  heading.className = "pc-product-history-state__title";
  heading.textContent = title;

  const body = document.createElement("p");
  body.className = "pc-product-history-state__message";
  body.textContent = message;
...
function createSummaryItem({ label, value, className = null }) {
  ...
  item.className = ["pc-product-history-summary__item", className].filter(Boolean).join(" ");

  const labelElement = document.createElement("span");
  labelElement.className = "pc-product-history-summary__label";
  labelElement.textContent = label;

  const valueElement = document.createElement("span");
  valueElement.className = "pc-product-history-summary__value";
...
function createCollapsedEventSummary(event, visibleDetails) {
  ...
  summaryButton.className = "pc-product-history-list__summary";
  ...
  chevron.scale = "s";
  chevron.className = "pc-product-history-list__chevron";
  ...
  detailsPanel.className = "pc-product-history-list__details-panel";
...
function createStaticEventSummary(event) {
  const container = document.createElement("div");
  container.className = "pc-product-history-list__summary pc-product-history-list__summary--static";
...
function createEventSummaryContent(event) {
  ...
  content.className = "pc-product-history-list__summary-content";

  const header = document.createElement("span");
  header.className = "pc-product-history-list__header";

  const title = document.createElement("span");
  title.className = "pc-product-history-list__title";
  title.textContent = event.title ?? "History event";

  const meta = document.createElement("span");
  meta.className = "pc-product-history-list__meta";
  ...
  if (event.description) {
    ...
    description.className = "pc-product-history-list__description";
...
function createEventDetails(details) {
  ...
  list.className = "pc-product-history-list__details";
