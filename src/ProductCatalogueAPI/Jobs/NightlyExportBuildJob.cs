...
/// </summary>
public sealed class NightlyExportBuildJob(IProductWorkflowRepository workflowRepository, IExportDecisionRuleSetRegistry ruleSets, IExportOperationService exportOperations, IDatasetLockService datasetLockService, TimeProvider timeProvider, ILogger<NightlyExportBuildJob> logger) : IBackgroundJob
{
    ...
    private readonly ILogger<NightlyExportBuildJob> _logger = logger;
    ...
    private async Task ProcessSummaryAsync(Data.Models.ProductChangeSummary summary, CancellationToken cancellationToken) {
        ...
        if (!decision.RevisionType.HasValue) {
            _logger.LogInformation("Nightly export deferred by ruleset. DatasetName: {DatasetName}. ProductSpecification: {ProductSpecification}. Reason: {Reason}.", summary.DatasetName, summary.ProductSpecification, decision.Reason);
            return;
        }
        ...
        if (datasetLock is null) {
            _logger.LogWarning("Nightly export skipped because its independent track lock is held. DatasetName: {DatasetName}. ProductSpecification: {ProductSpecification}.", summary.DatasetName, summary.ProductSpecification);
            return;
        }
