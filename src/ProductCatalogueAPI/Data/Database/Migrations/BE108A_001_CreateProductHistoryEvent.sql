-- BE108A_001: additive audit foundation. Never drop this table on application rollback.
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET ANSI_NULLS ON;
...
IF @@TRANCOUNT <> 0
    THROW 51079, 'BE108A: run create outside an existing transaction.', 1;
BEGIN TRY
    BEGIN TRANSACTION;
...
IF OBJECT_ID(N'dbo.ProductHistoryEvent') IS NOT NULL AND OBJECT_ID(N'dbo.ProductHistoryEvent', N'U') IS NULL
    THROW 51084, 'BE108A: an incompatible dbo.ProductHistoryEvent object already exists.', 1;
IF OBJECT_ID(N'dbo.ProductHistoryEvent', N'U') IS NULL
BEGIN
    IF OBJECT_ID(N'dbo.PK_ProductHistoryEvent') IS NOT NULL OR OBJECT_ID(N'dbo.UQ_ProductHistoryEvent_OperationId') IS NOT NULL
        THROW 51093, 'BE108A: an expected constraint name is already in use.', 1;
    -- Defer DDL binding until the absence check has succeeded.
    EXEC sys.sp_executesql N'CREATE TABLE [dbo].[ProductHistoryEvent]
    (
        [Id] uniqueidentifier NOT NULL,
        [OperationId] uniqueidentifier NOT NULL,
        [StateRecordId] uniqueidentifier NULL,
        [DatasetName] nvarchar(256) NOT NULL,
        [EventType] nvarchar(64) NOT NULL,
        [Outcome] nvarchar(32) NULL,
        [Code] nvarchar(128) NULL,
        [SafeMessage] nvarchar(1024) NULL,
        [CorrelationId] nvarchar(128) NULL,
        [JobId] nvarchar(128) NULL,
        [ExportTarget] nvarchar(32) NULL,
        [OperationMetadataJson] nvarchar(4000) NULL,
        [CreatedAtUtc] datetime2(7) NOT NULL,
        [UpdatedAtUtc] datetime2(7) NOT NULL,
        [ExecutionStartedAtUtc] datetime2(7) NULL,
        [FinalizedAtUtc] datetime2(7) NULL,
        [OccurredAtUtc] datetime2(7) NULL,
        CONSTRAINT [PK_ProductHistoryEvent] PRIMARY KEY CLUSTERED ([Id] ASC),
        CONSTRAINT [UQ_ProductHistoryEvent_OperationId] UNIQUE NONCLUSTERED ([OperationId] ASC)
    );
    CREATE NONCLUSTERED INDEX [IX_ProductHistoryEvent_DatasetName_OccurredAtUtc]
        ON [dbo].[ProductHistoryEvent] ([DatasetName] ASC, [OccurredAtUtc] DESC);
    CREATE NONCLUSTERED INDEX [IX_ProductHistoryEvent_Pending]
        ON [dbo].[ProductHistoryEvent] ([UpdatedAtUtc] ASC, [OperationId] ASC)
        INCLUDE ([FinalizedAtUtc]) WHERE [FinalizedAtUtc] IS NULL;';
END;
...
BEGIN CATCH
    ...
    THROW;
END CATCH;
PRINT 'BE108A_001 create and contract verification succeeded.';
