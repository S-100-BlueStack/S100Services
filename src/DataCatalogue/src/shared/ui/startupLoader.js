...
export function createStartupLoader() {
  ...
  element.className = "data-catalogue-startup-loader";
  element.setAttribute("role", "status");
  element.setAttribute("aria-live", "polite");
  element.setAttribute("aria-label", "DataCatalogue startup status");

  const cardElement = document.createElement("div");
  cardElement.className = "data-catalogue-startup-loader__card";

  const titleElement = document.createElement("h1");
  titleElement.className = "data-catalogue-startup-loader__title";
  titleElement.textContent = "DataCatalogue";

  const textElement = document.createElement("p");
  textElement.className = "data-catalogue-startup-loader__text";
  textElement.textContent = "Starting DataCatalogue...";

  const detailElement = document.createElement("p");
  detailElement.className = "data-catalogue-startup-loader__detail";
  detailElement.textContent = "";

  const progressElement = document.createElement("div");
  progressElement.className = "data-catalogue-startup-loader__progress";
  progressElement.setAttribute("role", "progressbar");
  progressElement.setAttribute("aria-valuemin", "0");
  progressElement.setAttribute("aria-valuemax", "100");

  const progressFillElement = document.createElement("div");
  progressFillElement.className = "data-catalogue-startup-loader__progress-fill";

  const progressLabelElement = document.createElement("p");
  progressLabelElement.className = "data-catalogue-startup-loader__progress-label";
  progressLabelElement.textContent = "";

  const actionButton = document.createElement("button");
  actionButton.type = "button";
  actionButton.className = "data-catalogue-startup-loader__action";
  ...
  function startLoading(text = "Starting DataCatalogue...") {
    cleanupTimers();
    retryHandler = null;
    actionButton.hidden = true;
  ...
  function complete({ text = "DataCatalogue ready." } = {}) {
    cleanupTimers();
  ...
  function fail({ text = "DataCatalogue could not be loaded.", message = "", onRetry } = {}) {
    cleanupTimers();
    element.hidden = false;
