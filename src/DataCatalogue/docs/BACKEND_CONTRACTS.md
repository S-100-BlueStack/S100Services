# DataCatalogue Backend Contracts

This document tracks backend assumptions, draft contracts, open questions and integration decisions for DataCatalogue.

Backend development is owned by the colleague. This scoped frontend baseline does not verify API readiness or contracts. Do not treat any draft below as final until it has been checked against the supplied backend contract.

Earlier phase notes preserve the previous Jobs/AOI assumptions and are historical. References in those notes to an unavailable or future backend do not establish its current implementation status. The role split below and section 13 record the accepted DataCatalogue direction.

## 1. Current backend status

Status: Integration contracts unverified; no API implementation changes in this candidate.

The exported repository inventory includes `src/DataCatalogueAPI`, but API implementation bodies are not part of this frontend contract review.
...
For the live direct-browser test map, verify the configured MapServer layer metadata and expected DK1-DK5 bindings to layers 4-8, browser origin/CORS support, supported runtime auth, expiry/refresh/cancellation behavior and safe user-facing failures. A MapServer path must not be silently rewritten to an assumed FeatureServer path.
